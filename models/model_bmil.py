...
class probabilistic_MIL_Bayes_enc(nn.Module):
    ...
    def forward(self, h, return_features=False, slide_label=None):
        ...
        postr_alpha = slide_label.detach() * (self.sf_pos * torch.softmax(postr_alpha / 0.1, dim=1)).clamp(min=1.0) + (1. - slide_label).detach() * (self.sf_neg * torch.softmax(postr_alpha / 5., dim=1)).clamp(max=0.95)
        print(1. - slide_label)
