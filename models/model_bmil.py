...
class probabilistic_MIL_Bayes_enc(nn.Module):
    ...
    def forward(self, h, return_features=False, slide_label=None):
        ...
        print('component 1: ', slide_label * (self.sf_pos * torch.softmax(postr_alpha / 0.1, dim=1)))
        print('component 2: ', (1. - slide_label) * (self.sf_neg * torch.softmax(postr_alpha / 5., dim=1)).clamp(max=0.95))

        postr_alpha = slide_label.detach() * (self.sf_pos * torch.softmax(postr_alpha / 0.1, dim=1)) \
